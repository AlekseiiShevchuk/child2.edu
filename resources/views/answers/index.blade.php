...
@section('content')
    ...
    @can('answer_create')
    <p>
        <a href="{{ route('answers.create') }}" class="btn btn-success">@lang('quickadmin.add_new')</a>
    </p>
    @endcan
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped {{ count($answers) > 0 ? 'datatable' : '' }} dt-select">
                <thead>
                    <tr>
                        @can('answer_delete')
                    ...
                    @if (count($answers) > 0)
                        @foreach ($answers as $answer)
                            <tr data-entry-id="{{ $answer->id }}">
                                ...
                                <td>{{ $answer->slide->title or '' }}</td>
                                <td>
                                    @can('answer_view')
                                    <a href="{{ route('answers.show',[$answer->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.view')</a>
                                    @endcan
                                    @can('answer_edit')
                                    <a href="{{ route('answers.edit',[$answer->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.edit')</a>
                                    @endcan
                                    @can('answer_delete')
                                    {!! Form::open(array(
